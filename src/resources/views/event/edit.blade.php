    ...
    <form action="{{ route('event.update') }}" method="post">
        ...
        <input type="hidden" name="event_id" value="{{ $event->event_id }}">
            ...
            <div class="form-grou mt-1 mb-4">
                ...
                <input type="text" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }} mt-1" name="title" id="title" value="{{ old('title', $event->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group w-50 mb-4">
                ...
                <select name="category_id" id="category_id" class="form-control{{ $errors->has('category_id') ? 'is-invalid' : '' }} mt-1">
            ...
            <div class="form-group w-25 mb-4">
                ...
                <input type="date" class="form-control{{ $errors->has('date') ? ' is-invalid' : '' }} mt-1" name="date" id="date" value="{{ old('datę', $event->date)}}">
                @if($errors->has('date'))
                <span class="invalid-feedback" role="alert">
                    {{ $errors->first('date') }}
                </span>
            ...
            {{-- もくもく会開催時間 --}}
            <div class="form-group w-50 mb-2">ωω
                <div class="row">
                    {{-- 開始時間 --}}
                    <div class="col">
                        <label for="start_time">{{ '開始時間' }}<span class="badge bg-danger ms-2">{{ '必須' }}</span></label>
                        <input type="time" class="form-control{{ $errors->has('start_time') ? ' is-invalid' : '' }} mt-2 mb-2" name="start_time" id="start_time" value="{{ old('start_time', $event->start_time) }}">
                    ...
                    <div class="col">
                        ...
                        <input type="time" class="form-control{{ $errors->has('end_time') ? ' is-invalid' : '' }} mt-2 mb-2" name="end_time" id="end_time" value="{{ old('end_time', $event->end_time) }}">
